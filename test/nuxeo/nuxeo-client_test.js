describe('ngNuxeoClient module', function () {
  ...
  describe('nuxeo service', function () {
    ...
    it('should query nuxeo server in right path when requested', function () {
      httpBackend.whenGET('http://demo.nuxeo.local/nuxeo/site/api/v1/user/Administrator').respond(dataUser);

      httpBackend.whenGET('http://demo.nuxeo.local/nuxeo/site/api/v1/query' +
        '?query=SELECT%20*%20FROM%20Document%20WHERE%201=1%20AND%20(dc:expired%20IS%20NULL%20OR%20dc:expired%20%3E=%20DATE%20\'2015-08-04\')%20' +
    ...
    it('should query nuxeo server in user path when requested', function () {
      httpBackend.whenGET('http://demo.nuxeo.local/nuxeo/site/api/v1/user/Administrator').respond(dataUser);
